...
```{r solution1}
lab1_titerdata <- read.csv("../data/03_hemisphering_lab1.csv", row.names = "X")
...
view(lab1_map,  options=list(xlim=c(-8,8), ylim=c(-8,8)))
...
```{r solution2}
lab2_titerdata <- read.csv("../data/03_hemisphering_lab2.csv", row.names = "X")
...
view(pc,  options=list(xlim=c(-8,8), ylim=c(-8,8)))

```

Well shoot. Something is definitely wrong. This lab's data is predicting
that majority of the yellow cluster and purple cluster is actually
...
uncertain. After thinking about this for an hour, Mappe decides to leave
...
of the labs to repeat the yellow antigens' whole titration set (never do
...
```{r solution3}
lab2_titerdata_repeat <- read.csv("../data/03_hemisphering_lab2_repeat.csv", 
                                  row.names = "X")
...
view(pc_repeat,  options=list(xlim=c(-8,8), ylim=c(-8,8)))
...
to have sera near them, maybe their position is less uncertain". What if
...
times as one wishes to his/her heart's content and limitations of
his/her mortality).
