...
Working code is provided for each section in the file `02_map_diagnostics_1_solution.Rmd`. We encourage you to try and write your own code here and to perform each task before runnning the provided code.
...
```{r}
map_unop = Racmacs::read.acmap("../data/maps/01_sars_cov_2_map_not_opt.ace")
```
...
map_op_5 = Racmacs::optimizeMap(
  map_unop,
  number_of_dimensions = 2,
  number_of_optimizations = 5
  ) %>%
  rotateMapCorrectly()
Racmacs::allMapStresses(map_op_5)
```
...
```{r}
map_op_10000 = Racmacs::optimizeMap(
  map_unop,
  number_of_dimensions = 2,
...
Racmacs::allMapStresses(map_op_10000)[1:50]
```
...
```{r}
plot(procrustesMap(map_op_5, map_op_10000))
```
...
(Note: you can use the second argument of rotateMapCorrectly to speciy which optimisations to rotate)

```{r}

Racmacs::view(
  rotateMapCorrectly(map_op_5, optimisations_to_rotate = 1:5),
  num_optimizations = 5
  )
...
source("helpful_functions/getFittedTiters.R")
fitted_titers = getFittedTiters(map_op_10000)

ggplot(fitted_titers) +
  geom_abline(
    intercept = 0 ,
    slope = ,
    linetype = "dashed",
...
```{r}
sd(fitted_titers$measured_titer - fitted_titers$fitted_titer)
```
...
Another valuable way to evaluate a model is cross-validation, which assesses _out-of-sample_ predictive performance. In goodness-of-fit assessments, we use the whole titer table to make a map, and checked whether the map accurately represents those titers. To perform cross-validation, we check how accurrately the map can be used to predict the value of measurements which were not used to produce the map. To achieve this, we:
...
```{r}
dimension_test = dimensionTestMap(
  map_op_10000, 
  dimensions_to_test = 1:3, 
...
Note that bootstrapping the map requires us to re-optimise the map many times, so is slow. The number of reoptimisaitons is set by the `bootstrap_repeats` parameter — today, we can be lazy with this and set it to 100 to save time — but when doing bootsrapping in "real" analysis, its important to set this to a suitably high number (e.g. 1000). To save time, you can run the bootstrapping, and save the output with `Racmacs::save.acmap`, rather than re-running the boostrapping step every time you run your script.

```{r}

bs_map_op_10000 = bootstrapMap(
  map_op_10000,
  method = "noisy",
...
bb_map_op_10000 = bootstrapBlobs(
  bs_map_op_10000,
  conf.level = 0.68,
  gridspacing = 0.05
)

srOpacity(bb_map_op_10000) = 0.2
...
map_missing_sera = Racmacs::read.acmap("../data/maps/02_sars_cov_2_map_missing.ace")
...
Both types of uncertainty interval are now _much_ larger for XBB.1.5 and its relatives, and for JN.1 and its relatives. Notice that the uncertainy intervals are bannana-shaped in the missing-sera map. We can understand this better by looking at the map in the interactive viewer. Open the interactive viewer on the full map with triangulation blobs, then click on the JN.1 antigen, then press ctrl+c to turn on "connection lines". These are lines which connect together antigen-serum pairs for which there is a measurement.


```{r}
Racmacs::view(triangulationBlobs(map_op_10000, grid_spacing = 0.05))
```

Do the same for the map with missing sera:

```{r}
Racmacs::view(triangulationBlobs(map_missing_sera, grid_spacing = 0.05))
```
...
```{r}
map_missing_sera_with_BA275_BQ11 = Racmacs::read.acmap("../data/maps/02_sars_cov_2_map_missing_plus_BA275_BQ11.ace")

plot(
  triangulationBlobs(
    map_missing_sera_with_BA275_BQ11,
    grid_spacing = 0.05
    )
)
```
...
```{r}
map_missing_sera_with_XBB15_JN1 = Racmacs::read.acmap("../data/maps/02_sars_cov_2_map_missing_plus_XBB15_JN1.ace")

plot(
  triangulationBlobs(
    map_missing_sera_with_XBB15_JN1,
    grid_spacing = 0.05
    )
)
```
...
```{r, message=FALSE}
map_no_triplicate = Racmacs::read.acmap("../data/maps/02_sars_cov_2_map_no_triplicate.ace")

boot_fullmap = bootstrapBlobs(
  bootstrapMap(
    map_op_10000,
    method = "noisy",
    ag_noise_sd = 0,
...
resample_bb = bootstrapBlobs(
   resample_bs,
   gridspacing = 0.05 
  )

noisy_bs = bootstrapMap(
      map_op_10000,
      method = "noisy",
      bootstrap_repeats = 1000,
      optimizations_per_repeat = 100,
...
As you can see, the resample boostrap produces wider uncertainty blobs than the noisy boostrap for this map — because it additional incorporates variation stemming from the fact that the different sera "see" the antigens subtley differently (and vice versa), so the choice of antigens and sera to including in the map affects their conformation.
