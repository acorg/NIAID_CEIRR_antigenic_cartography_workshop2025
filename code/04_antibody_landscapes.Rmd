...
This script contains tasks to construct antibody landscapes on top of antigenic maps. Working code is 
provided in [`code/04_antibody_landscapes_solution.Rmd`](./code/04_antibody_landscapes_solution.Rmd). 

We will be mainly working with simulated SARS-CoV-2 data. Much of the content was previously made available here: https://doi.org/10.5281/zenodo.13471558

Information on the ablandscapes package can be found here: https://rdrr.io/github/acorg/ablandscapes/

For plotting html objects, we will use the r3js package: https://cran.r-project.org/web/packages/r3js/index.html
...
Given that map distances are based on fold changes from the maximum titer variant in a serum, making maps from cross-reactive multi-exposure sera condenses maps and overshadows antigenic differences between variants. 
...
The *transposed* raw titer table, not the logtitertable, with "<LOD" (below limit of detection) titers written as "<LOD" is given as input. Titers that are not measured should be passed as "*". The column names are the antigens, the row names are the sera to be measured. The table is passed as a matrix. The `error.sd` parameter determines the standard deviation of the normal distribution used to 
evaluate the likelihood of measuring titer x with the current parameter set. A lower `error.sd` 
forces the fit to match the measured data more closely, potentially resulting in overfitting. The parameter 
...
The small variation from the expected values (1, 0.8, 0.4) can be explained by the fact that the base map contains other serum groups that determine the variants' positions and random noise in the titrations.
...
multiple hills and vallyes, rather than a single hill. [Fonville et al. (2014)](https://www.science.org/doi/10.1126/science.12564277)) modelled these 
...
flu_gmts <- 
```
Above is the influenza A/H3N2 base map, coloured by antigenic cluster. We will put the flu landscape on top with the 
same parameters as used in the publication (bandwith = 11, degree = 1) (SOM of [Fonville et al. (2014)](https://www.science.org/doi/10.1126/science.12564277)). You can look at the effect of changing both parameters in the example below by changing the bandwidth or degree.
