...
available in `code/04_antibody_landscapes_solutions.Rmd`. 
...
rbind(matrix_to_long(ba2_reinf, "BA.2 reinfection"),
      matrix_to_long(single_wu1, "Wu1 infection"),
...
titertables <- list("Wu1 infection" = titerTable(wu1_single_map), 
...
The small variation from the expected values (1, 0.8, 0.4) can be explained by the fact that the base map contains other serum groups that determine the variants' positions. For example, if we use the Wu1 only map from above as base map, we can see that the slopes match our input values:
...
```{r}
lndscp_fits$`Wu1 infection`$residuals
```
...
for Wu1 infected and Delta Breakthrough samples. We remove these antigens from the BA.1 reinfected titertables.
...
The remaining antigens were enough to fit very similar slopes to the full data set. *Importantly, removing the Wu1 
...
For the Wu1 infected and Delta Breakthrough samples, titer values were fitted also for the not measured antigens that were present in 
