...
This is an example based that was previously published in [Rössler, et al. Cell Reports (2025)](https://doi.org/10.1016/j.celrep.2024.115140.) .
...
evolution of SARS-CoV-2: BA.2.75 subvariants CH.1.1 and DV.7.1 escape single exposure sera to a similar degree as JN.1 subvariants KP.2, KP.3 and KZ.1.1. 
...
Potentially, both conformations are present in 3 dimensions and it is an effect of projecting a 3D map downwards onto the 2D plane:
...
We understand now that the two similar optima are due to downardsprojection of the 3D map onto 2 dimensions. Both local map optima are present in 
...
How do we resolve the conflict in the data to obtain a stable map that represents antigenic relationships?
...
BA.2, suggesting that removing CH.1.1 is the more reasonable choice, as done in the original publication.
