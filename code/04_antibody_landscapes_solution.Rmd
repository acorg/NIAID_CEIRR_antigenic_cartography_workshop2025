...
for (j in seq_len(nrow(coords))) {
  data3js <- r3js::lines3js(
          ...
          col = "grey50",
          toggle = sprintf("GMT, %s", "Wu1 infection"),
          geometry = TRUE,
          opacity = 0.7,
          lwd = 0.2 
        )
...
data3js <- lndscp3d_surface(
      data3js = data3js,
      object = lndscp_fits$`Wu1 infection`,
      crop2chull = FALSE,
      toggle = sprintf("GMT landscape, %s", "Wu1 infection"),
      grid_spacing = 0.5,
      padding = 0.2,
      options = list(
        col.surface = "blue",
...
fitted_vals_sub$`BA.2 reinfection` <- fitted_vals_sub$`BA.2 reinfection`[names(fitted_vals_sub$`Wu1 infection`)]
