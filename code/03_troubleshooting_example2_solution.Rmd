...
Let's plot the titers by exposure history:
...
We don't have additional information there. But we know that multi-exposure sera have higher cross-reactivity compared to single exposure sera and therefore decrease antigenic distances in a map. Hence, they should be removed from the map. Let's check the impact of the unknown sera on the map configuration and map stress. 

To do that, we start by colouring based on serum group:

```{r}
ag_colors <- readRDS(file.path(here::here(), "data", "01_sars_cov_2_antigen_colors.RDS"))
...
```{r}
ag_colors[c("Unknown 1", "Unknown 2")] <- c("#ff0000", "#0000ff")
...
The error lines show that the central sera unknown sera pull all variants closer to the middle.
...
Colouring by stress also shows that these sera are the most stressed in the map. It is time to remove them:
...
Most of the Day 4 sera are <LOD against the majority of variants, some don't have highest titers against their homologous variant (Wu.1 Day 4). Such patterns can occur when the antibody response did not have enough time 
...
ag_colors[c("Wu-1 Day 4", "BA.2 Day 4", "JN.1 Day 4")] <- c("#ff0000", "#0000ff", "green")
...
positioning, but we see that JN.1 and B.1.621 were pulled towards the center of the map.
