...
```{r load_data}
titerdata <- read.csv("../data/titerdata/01_artificial_titerdata.csv", row.names = "X")
```
...
The antigenic map algorithm transforms raw data into a dissimilarity matrix and performs multidimensional scaling (MDS) to preserve the relationships within the data to position the points in a lower dimension.
...
The RacViewer shows in addition to the absolute map stress in the bottom left corner also the stress per titer, followed by the stress per detectable titer in square brackets. This makes maps with different numbers of objects comparable, as including more sera and antigens in a map will increase the absolute map stress.
...
What do you think contributes to the map stress, i.e. error between titer distances and map distances? Is it the serum reactivity or the measurement noise? You can find out by simulating data with either effect. The viewer shows the total map stress, followed by the stress per titer and in square brackets the stress per detectable titer.
...
Above, we have been using quite "unrealistic" simulated data. Now, we'll produce and style a map using a titer table which looks more like something you might produce in practice, containing a selection of SARS-CoV-2 variants. We'll also use this map later on in the map diagnostics session.

Read in the titer table, `data/titerdata/01_sars_cov_2_titerdata.csv`":
```{r}
titers = read.csv("../data/titerdata/01_sars_cov_2_titerdata.csv", row.names = 1)
```
...
```{r}
antigen_colors = readRDS("../data/other/01_sars_cov_2_antigen_colors.RDS")
Racmacs::agFill(sars_cov_2_map) = antigen_colors[Racmacs::agNames(sars_cov_2_map)]
```
...
```{r}
dna_sequences_read = seqinr::read.fasta(file = "../data/other/01_spike_sequences.fasta")
```
...
Racmacs::agSequences(sars_cov_2_map_oriented) = aa_sequences_read[agNames(sars_cov_2_map_oriented),]
```
...
```{r}
Racmacs::view(sars_cov_2_map_oriented)
```
